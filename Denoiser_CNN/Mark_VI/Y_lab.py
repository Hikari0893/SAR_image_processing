...
checkpoint_callback = ModelCheckpoint(
        ...
        filename = f"Y_relu_{batch_size}_{function}_{epochs:2d}",
        save_top_k = -1,
        every_n_epochs = 1
    )
...
class SEblock_Unet(nn.Module):
    ...
    def __init__(self, width=256, height=256):
        ...
        self.dec_ups0  =  nn.Upsample(scale_factor=2, mode= 'bilinear')
        self.dec_conv0 = nn.ConvTranspose2d(768, 256, 3, stride=1, padding=1, output_padding=0) # (Batch, 256, 8, 8)
        
        self.dec_ups1  =  nn.Upsample(scale_factor=2, mode= 'bilinear')
        self.dec_conv1 = nn.ConvTranspose2d(512, 256, 3, stride=1, padding=1, output_padding=0) # (Batch, 512, 16, 16)
        
        self.dec_ups2  =  nn.Upsample(scale_factor=2, mode= 'bilinear')
        self.dec_conv2 = nn.ConvTranspose2d(384, 128, 3, stride=1, padding=1, output_padding=0) # (Batch, 128, 32, 32)
        
        self.dec_ups3  =  nn.Upsample(scale_factor=2, mode= 'bilinear')
        self.dec_conv3 = nn.ConvTranspose2d(256, 128, 3, stride=1, padding=1, output_padding=0) # (Batch, 128, 64, 64)
        
        self.dec_ups4  = nn.Upsample(scale_factor=2, mode= 'bilinear') # (Batch, 64, 128, 128)
        self.dec_conv4 = nn.ConvTranspose2d(192, 64, 3, stride=1, padding=1, output_padding=0) # (Batch, 64, 128, 128)
        
        
        self.dec_ups5  = nn.UpsamplingBilinear2d(scale_factor=2) # (Batch, 64, 256, 256)
        
        self.dec_conv6 = nn.Conv2d(96, 32, 3, stride=1, padding=1) # (Batch, 32, 256, 256)
        
        self.dec_output = nn.Conv2d(32, 1, 3, padding=1) #(Batch, 1, 256, 256)
    ...
    def forward(self, input):
        ...
        x = F.relu(self.dec_conv0(x)) # (Batch, 256, 8, 8)
        
        x = self.dec_ups1 (x)
        x = torch.cat([x, skips.pop()], dim=1)
        x = F.relu(self.dec_conv1(x) ) # (Batch, 256, 16, 16)
        
        x = self.dec_ups2 (x)
        x = torch.cat([x, skips.pop()], dim=1)
        x = F.relu(self.dec_conv2(x)) # (Batch, 128, 32, 32)
        
        x = self.dec_ups3 (x)
        x = torch.cat([x, skips.pop()], dim=1)
        x = F.relu(self.dec_conv3(x)) # (Batch, 64, 64, 64)
        
        x = self.dec_ups4 (x)
        x = torch.cat([x, skips.pop()], dim=1)
        x = F.relu(self.dec_conv4(x)) #(Batch, 32, 128, 128)
...
class Autoencoder_Y (pl.LightningModule,NPYDataLoader):
    def __init__(self, width = 256, hight = 256):
        pl.LightningModule.__init__(self)
        NPYDataLoader.__init__(self, batch_size=batch_size, num_workers=num_workers, folder_A = input_folder_A, folder_B = input_folder_B, only_test=only_test)
        self.lr = learning_rate
        self.loss_f = Loss_funct()
        self.SEB_UNET    = SEblock_Unet()
        # No additional layers needed here, as there is no processing, for now
        
    def forward (self, input):
        
        # Simply return the input as is
        return self.SEB_UNET(input)
...
if __name__ == '__main__':
    ...
    logger = TensorBoardLogger("tb_logs", name=f"Y_relu_{batch_size}_{function}_{epochs}")
