...
checkpoint_callback = ModelCheckpoint(
        ...
        filename =f"Wilson_net_{select}_{function}_{batch_size}_{epochs}",
        save_top_k = -1,
        every_n_epochs = 5
    )
...
class my_Unet(nn.Module):
    ...
    def forward (self, input):
        ...
        self.N = self.dec_output(decoder_6)  #(1,256,256)
...
class Autoencoder_Wilson_Ver1_1 (pl.LightningModule,NPYDataLoader):
    def __init__(self, width = 256, hight = 256):
...
if __name__ == '__main__':
    ...
    logger = TensorBoardLogger("tb_logs", name=f"Wilson_Net11_{select}_{function}_{batch_size}_{epochs}")
    ...
    trainer = Trainer(logger=logger, fast_dev_run=False, accelerator='gpu',
                      ...
                      max_epochs=epochs)
    Wilson_Ver11_Net = Autoencoder_Wilson_Ver1_1()
       
    trainer.fit(Wilson_Ver11_Net)#, ckpt_path=ckpt_path
