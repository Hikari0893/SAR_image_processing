...
checkpoint_callback = ModelCheckpoint(
        ...
        filename =f"WilsonVerGaussian_Net_{select}_{function}_{batch_size}_{epochs}",
        save_top_k = -1,
        every_n_epochs = 1
    )
...
class my_Unet(nn.Module):
    def __init__(self, width = 256, hight = 256):
        ...
        self.dec_ups  = nn.UpsamplingBilinear2d(scale_factor=2) # (32, 64, 128, 128)
...
class Autoencoder_Wilson_Gaussian (pl.LightningModule,NPYDataLoader):
    def __init__(self, width = 256, hight = 256):
        ...
        self.loss_f = nn.GaussianNLLLoss()
    ...
    def common_step(self,batch):
        ...
        X_normalize = self.normalize(X_input)
        Y_normalize = self.normalize(Y_reference)
        #Y_denormalized = Y_reference*(self.M-self.m) + self.m
          # Accede a self.N
    
        #neuronal network
        X_hat = self(X_normalize)
        log_Y = 2 * Y_normalize * (self.M - self.m) + self.m
        log_Xhat = 2 * X_hat * (self.M - self.m) + self.m
        #if self.Net.N is not None:
            #N = self.Net.N
        Xhat_var = log_Xhat.var(dim=(0, 2, 3), unbiased=False).unsqueeze(0).unsqueeze(2).unsqueeze(3)
        Xhat_var = Xhat_var.expand_as(X_hat)
        Xhat_mean = torch.mean(log_Xhat, dim=(0, 2, 3)).unsqueeze(0).unsqueeze(2).unsqueeze(3)
        Xhat_mean = Xhat_mean.expand_as(X_hat)
        
        Yref_mean = torch.mean(log_Y, dim=(0, 2, 3)).unsqueeze(0).unsqueeze(2).unsqueeze(3)
        Yref_mean = Yref_mean.expand_as(Y_reference)
        
        #loss function
        lossfuct = self.loss_f(Xhat_mean, Yref_mean, Xhat_var)
        return lossfuct
...
if __name__ == '__main__':
    ...
    logger = TensorBoardLogger("tb_logs", name=f"Wilson_Gaussian_{select}_{function}_{batch_size}_{epochs}")
    ...
    trainer = Trainer(logger=logger, fast_dev_run=False, accelerator='gpu',
                      ...
                      max_epochs=epochs)
    Wilson_Gaussian = Autoencoder_Wilson_Gaussian()
       
    trainer.fit(Wilson_Gaussian)#,ckpt_path=ckpt_path
