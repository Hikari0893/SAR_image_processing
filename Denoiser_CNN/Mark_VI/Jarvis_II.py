...
checkpoint_callback = ModelCheckpoint(
        ...
        filename =f"Wilson_Net3_{select}_{batch_size}_{epochs}",
        save_top_k = -1,
        every_n_epochs = 1
    )
...
class m_Unet(nn.Module):
    def __init__(self, width = 256, hight = 256):
        ...
        self.dec_ups  = nn.UpsamplingBilinear2d(scale_factor=2)#nn.Upsample(scale_factor=2, mode= 'nearest') # (32, 64, 128, 128)
    ...
    def forward (self, input):
        # Encoder
         # (32, 1, 256, 256)
        encoder_0 =self.activation_funct(self.enc_bn0(self.enc_conv0(input)), function) # (32, 32, 256, 256)
        skip = [encoder_0]
        ...
        decoder_4 = self.activation_funct(self.dec_ups(self.dec_bn4(self.dec_conv4(decoder_4))), function) #(64,128,128)
...
class Autoencoder_Wilson_Ver3 (pl.LightningModule,NPYDataLoader):
    def __init__(self, width = 256, hight = 256):
...
if __name__ == '__main__':
    ...
    logger = TensorBoardLogger("tb_logs", name=f"Wilson_Net3_{select}_{batch_size}_{epochs}")
    #Wilson_Ver3_Net = Autoencoder_Wilson_Ver3.load_from_checkpoint('/home/tonix/Documents/Dayana/mis_checkpoints/WilsonVer3_Net_select=0_function=0_epoch= 1-v1.ckpt')
    ...
    Wilson_Ver3_Net = Autoencoder_Wilson_Ver3()
    trainer.fit(Wilson_Ver3_Net)
