...
class DataGenerator():
    ...
    def load_imgs(self, files, dims='YX', max_files=None):
        ...
        for f in files[0:max_files]:
            ...
            if f.endswith('.h5') or f.endswith('.hdf5'):
            ...
            elif f.endswith('.npy') or f.endswith('.npy'):
                f = np.load(f, allow_pickle=True)  # Enable pickle for loading object arrays
                if isinstance(f, np.ndarray):
                    img = f
                else:
                    raise TypeError(f"Unexpected type in the .npy file: {type(f)}")
                img = np.square(np.abs(img)).astype(np.float32)
            else:
                if f.endswith('.tif') or f.endswith('.tiff'):
                    ...
                    raise Exception("JPEG is not supported, because it is not loss-less and breaks the pixel-wise independence assumption.")
                else:
                    raise Exception(f"Filetype '{f}' is not supported.")
